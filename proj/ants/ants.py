...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        dis = 0
        target = self.place
        while target.is_hive == False:
            if dis <= self.upper_bound and dis >= self.lower_bound and target.bees != []:
                return random_bee(target.bees) 
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.cooldown < 0 and self.place.bees:
            ...
            #print("DEBUG: bees,is_none=",self.place.bees,self.place.bees == None)
