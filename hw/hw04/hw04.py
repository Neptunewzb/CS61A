...
def balanced(m):
    ...
    m1 = end(a1)
    m2 = end(a2)
    if length(a1) * total_mass(m1) != length(a2) * total_mass(m2):
        return False
    if is_mobile(m1) and not balanced(m1):
        return False
    
    if is_mobile(m2) and not balanced(m2):
        return False
    return True
...
def berry_finder(t):
    ...
    True
    """
    #***HINT: to iterate through each branch of a tree, consider using a for loop to get each branch***
    if label(t) == 'berry':
        return True
    for b in branches(t):
        if berry_finder(b):
...
def max_path_sum(t):
    ...
    max_descendent = 0
    for b in branches(t):
        if max_path_sum(b) > max_descendent:
            max_descendent = max_path_sum(b)
    return label(t) + max_descendent
