...
class FreeChecking(Account):
    ...
    def __init__(self, account_holder):
        ...
        self.free_cnt = FreeChecking.free_withdrawals
    ...
    def withdraw(self, amount):
        if self.free_cnt > 0:
            ...
            flag = 0
        self.free_cnt = max(self.free_cnt-1, 0)
        if flag:
            if amount > self.balance:
                return "Insufficient funds"
            self.balance = self.balance - amount
...
def without(s, i):
    ...
    if i == 0:
        ...
        if ans.rest is not Link.empty:
            ans.rest = without(ans.rest, i-1)
    return ans
